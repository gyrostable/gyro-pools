...
def test_decimal_behavior(signed_math_testing, a, b, op_name):
    ...
    try:
        c = op(a, b)
        if (
                (op_name == "mul" and (a * b > D(MAX_INT//10**36) or a * b < D(MIN_INT//10**36)))
                or (op_name == "add" and (a + b > D(MAX_INT//10**18) or a + b < D(MIN_INT//10**18)))
                or (op_name == "div" and (a > D(MAX_INT//10**18) or a < D(MIN_INT//10**18)))
        ):
