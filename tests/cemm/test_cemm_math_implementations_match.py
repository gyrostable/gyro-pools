...
ApproxDecimal.__ge__ = lambda self, other: self.expected >= other.expected or self == other


@given(
    params=gen_params(),
    t=gen_balances_vector()
)
...
def test_zeta(params_px, gyro_cemm_math_testing):
    params, px = params_px  # Annoying manual unpacking b/c hypothesis is oddly limited at dependent arguments.
    mparams = params2MathParams(params)
...
def test_mkDerivedParams(params, gyro_cemm_math_testing):
    mparams = params2MathParams(params)
    derived_sol = mk_CEMMMathDerivedParams_from_brownie(gyro_cemm_math_testing.mkDerivedParams(scale(params)))
...
def test_virtualOffsets(params, invariant, gyro_cemm_math_testing):
    mparams = params2MathParams(params)
    derived_sol = mk_CEMMMathDerivedParams_from_brownie(gyro_cemm_math_testing.mkDerivedParams(scale(params)))
    ab_sol = gyro_cemm_math_testing.virtualOffsets(scale(params), derived_sol,  scale(invariant))
...
def test_maxBalances(params, invariant, gyro_cemm_math_testing):
    mparams = params2MathParams(params)
    derived_sol = mk_CEMMMathDerivedParams_from_brownie(gyro_cemm_math_testing.mkDerivedParams(scale(params)))
    xy_sol = gyro_cemm_math_testing.maxBalances(scale(params), derived_sol, scale(invariant))
...
def test_calculateInvariant(params, balances, gyro_cemm_math_testing):
    mparams = params2MathParams(params)
    derived_sol = mk_CEMMMathDerivedParams_from_brownie(gyro_cemm_math_testing.mkDerivedParams(scale(params)))
    uinvariant_sol = gyro_cemm_math_testing.calculateInvariant(scale(balances), scale(params), derived_sol)
...
def test_calcOutGivenIn(params, balances, amountIn, tokenInIsToken0, gyro_cemm_math_testing):
...
def test_calcInGivenOut(params, balances, amountOut, tokenInIsToken0, gyro_cemm_math_testing):
    ixIn = 0 if tokenInIsToken0 else 1
    ixOut = 1 - ixIn
...
def test_calculateSqrtOnePlusZetaSquared(params, balances, gyro_cemm_math_testing):
    ...
    assert val_explicit == val_implicit.approxed()  # Tests math / the python implementation
...
def test_liquidityInvariantUpdate(params_cemm_dinvariant, gyro_cemm_math_testing):
    ...
    rnew_sol = gyro_cemm_math_testing.liquidityInvariantUpdate(
        scale(balances), scale(params), derived_sol, scale(cemm.r), scale(deltaBalances), (dinvariant >= 0)
    )

    assert to_decimal(rnew_sol) == scale(rnew).approxed_scaled()

@given(params_cemm_dinvariant=gen_params_cemm_dinvariant())
def test_liquidityInvariantUpdateEquivalence(params_cemm_dinvariant, gyro_cemm_math_testing):
