...
def test_calculate_quadratic(gyro_two_math_testing):
    ...
    assert a * fresult**2 - b*fresult - c == pytest.approx(0)  # One should imply the other (up to approximation error), but let's double check.
...
def test_liquidityInvariant_Update(gyro_two_math_testing):
    ...
    for isIncreaseLiq in [True, False]:  # We test both increasing and decreasing liquidity.
...
def test_calc_out_given_in(gyro_two_math_testing):
    ...
    ## - dY = y'- l^2 / (x'+dX)
    expected_result = ((balanceOut+virtualParamOut) - ((currentInvariant/(10 **18))**2)/(balanceIn+virtualParamIn+amountIn)) * 10 **18
...
def test_calc_in_given_out(gyro_two_math_testing):
    ...
    expected_result = (((currentInvariant/(10 **18))**2)/(balanceOut+virtualParamOut-amountOut) -(balanceIn+virtualParamIn)) * 10 **18
